...
AirPods companion 자체는 이미 LibrePods, CAPod, OpenPods와 Podsify 같은 공개 대안이 있다. CAPod는
광고 없이 battery·popup·widget을 제공하고 LibrePods는 제어 기능까지 훨씬 넓으므로 일반 사용자는
이들을 먼저 비교하는 편이 맞다. 전체 비교는 [기존 프로젝트 조사](../docs/LANDSCAPE.md)에 있다.

직접 구현은 사용 중이던 별도 앱에서 연결 때 광고나 반복 안내를 겪은 뒤, 현재 기기에 필요한
battery·widget·지속 연결 카드와 DND 동작을 최소 권한으로 직접 검증하려는 개인 실험이었다. 더
완성된 범용 앱이라는 주장이 아니며 다른 앱의 소스나 디자인 자산을 복사하지 않았다.
