...
여러 번의 스와이프를 이어 붙이지 않고 한 번만 움직입니다. 그래서 이동 중 화면을 다시 만지면
Android의 일반 스크롤처럼 멈출 수 있습니다. X에서는 조금 더 멀리 움직이지만 아주 긴 피드의
처음까지 한 번에 닿지 않을 수 있습니다. 반복 동작을 사용하지 않는 이유는 화면이 다시 빨라지고,
사용자가 중간에 멈추려 해도 계속 움직이는 문제가 있었기 때문입니다.
